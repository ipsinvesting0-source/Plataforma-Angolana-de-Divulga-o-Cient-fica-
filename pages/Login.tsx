...
export const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({ title: 'Link de acesso enviado!', description: 'Verifique seu email para entrar.', type: 'success' });
    } catch (error: any) {
      addToast({ title: 'Erro no login', description: error.message, type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
      ...
      if (error) addToast({ title: 'Erro Google', description: error.message, type: 'error' });
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        <div className="text-center mb-8">
           ...
           <p className="text-gray-500">Acesse sua conta para gerenciar pesquisas.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input 
              ...
              placeholder="seu.email@instituicao.ao"
            />
          </div>
          <button 
            type="submit" 
        ...
        <div className="my-6 flex items-center">
            ...
            <span className="px-4 text-xs text-gray-400 uppercase">Ou</span>
            <div className="flex-1 border-t border-gray-200"></div>
        </div>
